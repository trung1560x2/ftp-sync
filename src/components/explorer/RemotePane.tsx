...
import { Folder, File, ArrowUp, RefreshCw, ChevronRight, Server } from 'lucide-react';
...
interface RemotePaneProps {
  ...
  onDropLocalItem: (item: DragDropLocalData) => void;
  onDownloadItem: (item: RemoteFileItem) => void;
  onDownloadMultiple: (items: RemoteFileItem[]) => void;
  onBulkRename: (dirPath: string, items: RemoteFileItem[]) => void;
  onChmod: (items: RemoteFileItem[]) => void;
  onArchive: (folderPath: string) => void;
  onExtract: (archivePath: string) => void;
  onDeleteMultiple: (items: RemoteFileItem[]) => void;
...
const RemotePane: React.FC<RemotePaneProps> = ({
  ...
  const [currentPath, setCurrentPath] = useState(targetDirectory || '/');
  ...
  const handleCalculateSize = async (item: RemoteFileItem) => {
    if (!sessionId) return;
    ...
    try {
      const res = await fetch(`/api/terminal/sessions/${sessionId}/dir-size?path=${encodeURIComponent(item.path)}`);
  ...
  const fetchDir = useCallback(async (dirPath: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) throw new Error(data.message || 'Failed to list remote directory');
      
      const mappedItems = (data.items || []).map((item: { name: string; isDirectory: boolean; size: number; modifiedAt?: string; fullPath?: string }) => ({
        name: item.name,
        isDirectory: item.isDirectory,
        size: item.size,
        modifiedAt: item.modifiedAt,
        path: item.fullPath || (dirPath.endsWith('/') ? dirPath + item.name : dirPath + '/' + item.name)
      }));
  ...
  useEffect(() => {
    if (targetDirectory && sessionId) {
      setCurrentPath(targetDirectory);
      fetchDir(targetDirectory);
    }
  }, [targetDirectory, sessionId, fetchDir, refreshTrigger]);
  ...
  const handleDragStart = (e: React.DragEvent, item: RemoteFileItem) => {
    const dragData: DragDropRemoteData = {
      type: 'remote',
      name: item.name,
      path: item.path,
      isDirectory: item.isDirectory,
      relPath: item.path.substring(targetDirectory.length).replace(/^\//, '')
    };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      if (!dataStr) return;
      const data = JSON.parse(dataStr) as DragDropLocalData;
      if (data.type === 'local') {
        onDropLocalItem(data);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSelectAll = () => {
    const filtered = items.filter(item => item.name !== '.' && item.name !== '..');
    if (selectedItems.size === filtered.length) {
      setSelectedItems(new Set());
    } else {
      setSelectedItems(new Set(filtered.map(f => f.path)));
    }
  };

  // Context Menu handlers
  const handleRightClick = (e: React.MouseEvent, item: RemoteFileItem) => {
    e.preventDefault();
    setContextMenu({
      x: e.clientX,
      y: e.clientY,
  ...
  const filteredItems = items.filter(item => item.name !== '.' && item.name !== '..');
  const pathSegments = currentPath.split('/').filter(Boolean);

  const getSelectedObjects = (): RemoteFileItem[] => {
    return filteredItems.filter(f => selectedItems.has(f.path));
  };
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-neutral-800 bg-neutral-950/80 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-neutral-200 uppercase tracking-widest text-[11px]">Remote Explorer</span>
        </div>
        <div className="flex items-center gap-1.5">
          <button 
            onClick={goUp}
            disabled={currentPath === '/'}
            className="p-1.5 text-neutral-500 hover:text-orange-500 hover:bg-neutral-800 rounded transition-colors disabled:opacity-30"
            title="Go Up"
          >
            <ArrowUp size={13} />
          </button>
      ...
      {/* Path Breadcrumb */}
      <div className="flex items-center gap-0.5 p-2 border-b border-neutral-800/40 bg-neutral-900/40 text-[10px] text-neutral-500 overflow-x-auto scrollbar-none flex-shrink-0">
        <span className="cursor-pointer hover:text-orange-400" onClick={() => navigateTo('/')}>/</span>
        {pathSegments.map((seg, i) => {
          ...
          return (
            <React.Fragment key={i}>
              ...
              <span 
                className="cursor-pointer hover:text-orange-400 truncate max-w-[80px]" 
                title={seg}
      ...
      {selectedItems.size > 0 && (
        <div className="flex items-center justify-between p-2 bg-orange-600/10 border-b border-orange-500/20 text-[10px] flex-shrink-0">
          ...
          <div className="flex gap-1.5">
            <button
              onClick={() => onDownloadMultiple(getSelectedObjects())}
            ...
            <button
              onClick={() => onChmod(getSelectedObjects())}
              className="px-2 py-0.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-bold uppercase rounded font-sans cursor-pointer"
            >
              Chmod
            </button>
              ...
              {filteredItems.map(item => {
                ...
                return (
                  <tr
                    ...
                    onDoubleClick={() => item.isDirectory ? navigateTo(item.path) : onOpenFile?.(item.path)}
      ...
      {contextMenu && (
        <div
          ...
          className="fixed z-50 bg-neutral-900 border border-neutral-800 rounded-lg shadow-2xl p-1 w-40 font-sans text-[11px] text-neutral-300"
        >
          {!contextMenu.item.isDirectory && onOpenFile && (
            <button
              onClick={() => {
                setContextMenu(null);
          ...
          <button
            ...
            className="w-full text-left px-3 py-1.5 hover:bg-neutral-800 hover:text-white rounded transition-colors cursor-pointer border-t border-neutral-800"
          >
            Download
          </button>
          <button
          ...
          {contextMenu.item.isDirectory ? (
            <>
              <button
                onClick={() => {
                  setContextMenu(null);
                  onArchive(contextMenu.item.path);
                }}
                className="w-full text-left px-3 py-1.5 hover:bg-neutral-800 hover:text-white rounded transition-colors border-t border-neutral-800 cursor-pointer"
              >
                Archive (Zip/Tar)
          ...
          ) : isArchiveFile(contextMenu.item.name) ? (
            <button
              onClick={() => {
                setContextMenu(null);
                onExtract(contextMenu.item.path);
              }}
              className="w-full text-left px-3 py-1.5 hover:bg-neutral-800 hover:text-white rounded transition-colors border-t border-neutral-800 cursor-pointer"
            >
              Extract Archive
