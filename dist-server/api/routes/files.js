...
const scanRemote = async (client, dir, base, recursive, ignoredFolders, depth = 0) => {
    if (depth > MAX_DEPTH)
        return [];
    try {
...
const scanLocal = async (dir, base, recursive, ignoredFolders, depth = 0) => {
    if (depth > MAX_DEPTH)
        return [];
    try {
        // Check if dir exists
...
router.get('/ftp/:id', async (req, res) => {
    const { id } = req.params;
