...
// Configuration interface received from main thread
...
const log = (type: 'info' | 'error', message: string) => {
    if (parentPort) parentPort.postMessage({ type: 'LOG', data: { type, message } });
};
...
async function runHelper() {
    ...
    parentPort.on('message', async (message) => {
        if (message.type === 'START_DIFF') {
            const data: DiffWorkerData = message.data;
            try {
...
async function scanRemote(client: TransferClient, dir: string, base: string, recursive: boolean, ignoredFolders: Set<string>, depth: number = 0): Promise<any[]> {
...
async function scanLocal(dir: string, base: string, recursive: boolean, ignoredFolders: Set<string>, depth: number = 0): Promise<any[]> {
    if (depth > MAX_DEPTH) return [];
    try {
...
async function performDiff(data: DiffWorkerData) {
